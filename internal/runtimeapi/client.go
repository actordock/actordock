...
type Client struct {
	...
	api  runtimeapipb.ControlClient
}
...
func Dial(addr string) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("dial runtime-api %s: %w", addr, err)
	}
	return &Client{
		conn: conn,
		api:  runtimeapipb.NewControlClient(conn),
	}, nil
}
...
func (c *Client) CreateAndResumeSandbox(
	...
	_, err := c.api.CreateActor(ctx, &runtimeapipb.CreateActorRequest{
		ActorId:                actorID,
	...
	_, err = c.api.ResumeActor(ctx, &runtimeapipb.ResumeActorRequest{
		ActorId: actorID,
	})
	if err != nil {
...
func (c *Client) actorBackend(actor *runtimeapipb.Actor, envdPort int) (string, error) {
	ip := actor.GetSandboxPodIp()
	if ip == "" {
...
func (c *Client) ResumeSandboxBackend(ctx context.Context, actorID string, envdPort int) (string, bool, error) {
	...
	resumeResp, err := c.api.ResumeActor(ctx, &runtimeapipb.ResumeActorRequest{
		ActorId: actorID,
	})
	if err != nil {
		if status.Code(err) == codes.NotFound {
...
func (c *Client) SuspendSandbox(ctx context.Context, actorID string) error {
	...
	if resp.GetActor().GetStatus() == runtimeapipb.Actor_STATUS_SUSPENDED {
		return nil
	}

	_, err = c.api.SuspendActor(ctx, &runtimeapipb.SuspendActorRequest{ActorId: actorID})
	if err != nil {
		if status.Code(err) == codes.NotFound {
			return ErrNotFound
		}
...
func (c *Client) DeleteSandbox(ctx context.Context, actorID string) error {
	...
	_, err := c.api.DeleteActor(ctx, &runtimeapipb.DeleteActorRequest{ActorId: actorID})
	if err != nil {
		if status.Code(err) == codes.NotFound {
			return ErrNotFound
		}
