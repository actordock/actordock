...
func TestBuildDeploymentApplyConfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wp: testWorkerPoolApplyConfig(&runtimev1alpha1.WorkerPoolPodTemplate{
...
// toleration on actordock.dev/sandboxClass); other classes get none of it.
func TestMicroVMPodShape(t *testing.T) {
	tests := []struct {
		name        string
		class       runtimev1alpha1.SandboxClass
		wantMicroVM bool
	}{
		{"gvisor default", "", false},
		{"gvisor explicit", runtimev1alpha1.SandboxClassGvisor, false},
		{"microvm", runtimev1alpha1.SandboxClassMicroVM, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			wp := testWorkerPoolApplyConfig(nil)
			...
			_, hasSelector := ps.NodeSelector["actordock.dev/sandboxClass"]
			hasTol := false
			for _, tol := range ps.Tolerations {
				if tol.Key != nil && *tol.Key == "actordock.dev/sandboxClass" {
					hasTol = true
				}
			}
...
func testWorkerPoolApplyConfig(tmpl *runtimev1alpha1.WorkerPoolPodTemplate) *runtimev1alpha1.WorkerPool {
...
func expectedDeploymentApplyConfig(mutatePodSpec func(*corev1ac.PodSpecApplyConfiguration)) *appsv1ac.DeploymentApplyConfiguration {
	...
	podSpecAC := corev1ac.PodSpec().
		...
		WithVolumes(corev1ac.Volume().
			WithName("run-runtime-sandbox").
			WithHostPath(corev1ac.HostPathVolumeSource().
				WithPath(sandboxpath.BasePath).
				WithType(corev1.HostPathDirectoryOrCreate))).
		WithContainers(corev1ac.Container().
			WithName("runtime-sandbox").
			WithImage(wp.Spec.SandboxImage).
			...
			WithVolumeMounts(corev1ac.VolumeMount().
				WithName("run-runtime-sandbox").
				WithMountPath(sandboxpath.BasePath)).
	...
	return appsv1ac.Deployment(deploymentName(wp.Name), wp.Namespace).
		WithOwnerReferences(metav1ac.OwnerReference().
			WithAPIVersion(runtimev1alpha1.GroupVersion.String()).
			WithKind("WorkerPool").
			WithName(wp.Name).
			WithUID(wp.UID).
			WithController(true).
		...
		WithSpec(appsv1ac.DeploymentSpec().
			...
			WithSelector(metav1ac.LabelSelector().
				WithMatchLabels(map[string]string{"actordock.dev/worker-pool": wp.Name})).
			WithTemplate(corev1ac.PodTemplateSpec().
				WithLabels(map[string]string{"actordock.dev/worker-pool": wp.Name}).
				WithSpec(podSpecAC)))
}
