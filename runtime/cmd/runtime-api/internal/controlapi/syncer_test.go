...
import (
	...
	"github.com/actordock/runtime/cmd/runtime-api/internal/store/storetest"
...
func TestSyncer_DeleteBoundWorker_ClearsActor(t *testing.T) {
	...
	if err := wait.PollUntilContextTimeout(ctx, 50*time.Millisecond, 2*time.Second, true, func(c context.Context) (bool, error) {
		...
		got = a
		return a.GetStatus() == runtimeapipb.Actor_STATUS_SUSPENDED, nil
	}); err != nil {
		t.Fatalf("actor not reset to SUSPENDED: %v", err)
	}
	if got.SandboxPodName != "" || got.SandboxPodNamespace != "" || got.SandboxPodIp != "" || got.InProgressSnapshot != "" {
