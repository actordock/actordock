...
const (
	routerServiceName = "runtime-net-router"

	// runtime-net.router.route.duration measures the latency from when the ext_proc handler receives a request
	// (Envoy -> EPP) until the target worker endpoint is resolved
	routeDurationMetricName = "runtime-net.router.route.duration"
)

// newRouteDurationHistogram creates the runtime-net.router.route.duration histogram from
...
func newRouteDurationHistogram() (metric.Float64Histogram, error) {
	h, err := otel.Meter(routerServiceName).Float64Histogram(
		...
		metric.WithDescription(
			"latency between Actordock router receiving a request and resolving "+
