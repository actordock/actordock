...
// are declared here.
func buildDeploymentApplyConfig(wp *runtimev1alpha1.WorkerPool) *appsv1ac.DeploymentApplyConfiguration {
		...
		WithSpec(appsv1ac.DeploymentSpec().
			...
			WithTemplate(corev1ac.PodTemplateSpec().
				WithLabels(map[string]string{
					"actordock.dev/worker-pool": wp.Name,
				}).
				WithSpec(podSpecAC)))
}
...
func maybeApplyMicroVMPodShape(
	...
	sandboxClass runtimev1alpha1.SandboxClass,
) {
	if sandboxClass != runtimev1alpha1.SandboxClassMicroVM {
		return
	}
	...
	// toleration on actordock.dev/sandboxClass=microvm. This is our own convention
	...
	podSpecAC.NodeSelector["actordock.dev/sandboxClass"] = string(runtimev1alpha1.SandboxClassMicroVM)
	podSpecAC.WithTolerations(corev1ac.Toleration().
		WithKey("actordock.dev/sandboxClass").
		WithOperator(corev1.TolerationOpEqual).
		WithValue(string(runtimev1alpha1.SandboxClassMicroVM)).
...
func applyWorkerPoolPodTemplate(
	podSpecAC *corev1ac.PodSpecApplyConfiguration,
	containerAC *corev1ac.ContainerApplyConfiguration,
	tmpl *runtimev1alpha1.WorkerPoolPodTemplate,
) {
	podSpecAC.NodeSelector = map[string]string{}
