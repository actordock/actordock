...
const (
	// IdentityMountPath is the in-actor directory at which runtime-worker bind-mounts
	...
	IdentityMountPath = "/run/actordock"
...
func prepareOCIDirectory(ctx context.Context, pullCache *memorypullcache.MemoryPullCache, actorTemplateNamespace, actorTemplateName, actorID, containerName, ref string, args []string, env []string, annotations map[string]string, netns string, identityDir string, durableDirVolumeMounts []*runtimeworkerpb.VolumeMount) error {
	...
	bundlePath := sandboxpath.OCIBundlePath(actorTemplateNamespace, actorTemplateName, actorID, containerName)
...
func buildActorOCISpec(actorTemplateNamespace string, actorTemplateName string, actorID string, args []string, env []string, annotations map[string]string, netns string, identityDir string, durableDirVolumeMounts []*runtimeworkerpb.VolumeMount) *specs.Spec {
	envVars := []string{
	...
	spec := &specs.Spec{
		...
		Linux: &specs.Linux{
			Namespaces: []specs.LinuxNamespace{
				...
				{
					Type: "network",
					Path: netns, // Will be created by runtime-sandbox
				},
				{
					Type: "ipc",
				},
	...
	for _, vm := range durableDirVolumeMounts {
		spec.Mounts = append(spec.Mounts, specs.Mount{
			...
			Type:        "bind",
			Source:      sandboxpath.DurableDirVolumeMountPoint(actorTemplateNamespace, actorTemplateName, actorID, vm.GetName()),
		})
	}
...
func untar(ctx context.Context, tarData io.Reader, rootPath string) error {
	tracer := otel.Tracer("runtime-sandbox")
