...
// and projects them onto the runtimeworkerpb.SandboxAssets runtime-worker fetches. It takes the
...
func resolveSandboxAssets(
...
) (*runtimeworkerpb.SandboxAssets, error) {
	...
	if class == "" {
		class = runtimev1alpha1.SandboxClassGvisor
	}

	var sc *runtimev1alpha1.SandboxConfig
...
func defaultSandboxConfig(lister listersv1alpha1.SandboxConfigLister, class runtimev1alpha1.SandboxClass) (*runtimev1alpha1.SandboxConfig, error) {
	all, err := lister.List(labels.Everything())
	if err != nil {
		return nil, fmt.Errorf("while listing SandboxConfigs: %w", err)
	}
	var match *runtimev1alpha1.SandboxConfig
	for _, sc := range all {
...
// sandboxAssetsProto converts a resolved SandboxConfig into the proto runtime-worker
// consumes.
func sandboxAssetsProto(class runtimev1alpha1.SandboxClass, sc *runtimev1alpha1.SandboxConfig) *runtimeworkerpb.SandboxAssets {
	out := &runtimeworkerpb.SandboxAssets{
		SandboxClass: string(class),
		Assets:       make(map[string]*runtimeworkerpb.ArchAssets, len(sc.Spec.Assets)),
	}
	for arch, files := range sc.Spec.Assets {
		archAssets := &runtimeworkerpb.ArchAssets{Files: make(map[string]*runtimeworkerpb.AssetFile, len(files))}
		for name, f := range files {
			archAssets.Files[name] = &runtimeworkerpb.AssetFile{Url: f.URL, Sha256: f.SHA256}
		}
