...
func (c *Controller) reconcile(ctx context.Context) error {
	...
	// 1. Get the ClusterIP of runtime-net-router in actordock-system namespace
	routerSvc := &corev1.Service{}
	if err := c.Client.Get(ctx, types.NamespacedName{Name: "runtime-net-router", Namespace: systemNamespace}, routerSvc); err != nil {
		if errors.IsNotFound(err) {
			slog.WarnContext(ctx, "runtime-net-router service not found, skipping until it is available")
			return nil
		}
	...
	if routerIP == "" || routerIP == "None" {
		slog.WarnContext(ctx, "runtime-net-router service has no ClusterIP yet, waiting...")
		return nil
	}

	// 2. Get the ClusterIP of dns service in actordock-system namespace
...
// reconcileKubeDNSConfig ensures that the kube-dns ConfigMap has a stub domain for actordock-system.
