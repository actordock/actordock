...
// resync events, and a counter for failed PUBLISH operations (in runtimeredis).
type Cache struct {
	store          store.Interface
	relistInterval time.Duration

	mu      sync.RWMutex
	workers map[string]*runtimeapipb.Worker
...
func New(store store.Interface, relistInterval time.Duration) *Cache {
	return &Cache{
		...
		workers:        make(map[string]*runtimeapipb.Worker),
	}
}
...
// expected to retry.
func (c *Cache) Workers() ([]*runtimeapipb.Worker, error) {
	if !c.ready.Load() {
		return nil, fmt.Errorf("worker cache not ready")
	}
	c.mu.RLock()
	defer c.mu.RUnlock()
	out := make([]*runtimeapipb.Worker, 0, len(c.workers))
...
func (c *Cache) relist(ctx context.Context) error {
	...
	newMap := make(map[string]*runtimeapipb.Worker, len(workers))
...
func workerKey(w *runtimeapipb.Worker) string {
