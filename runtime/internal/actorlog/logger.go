...
// by the gVisor and micro-VM sandbox runtimes. It forwards an actor container's
// stdout/stderr to the worker pod's stdout, annotated with actordock.dev/* labels, and
...
func (al *ActorLogger) EmitLifecycleLog(msg, actorID, actorTemplateName, actorTemplateNamespace string) {
	envelope := map[string]any{
		...
		al.labelsKey: map[string]string{
			"actordock.dev/actor_id":                 actorID,
			"actordock.dev/actor_template_name":      actorTemplateName,
			"actordock.dev/actor_template_namespace": actorTemplateNamespace,
		},
	}
...
// structured format. containerName is added as the actordock.dev/container_name label so
		...
		if len(lineBytes) > 0 {
			...
			if unmarshalErr != nil {
				labels := map[string]string{
					...
					"actordock.dev/actor_template_name":      actorTemplateName,
					"actordock.dev/actor_template_namespace": actorTemplateNamespace,
					"actordock.dev/container_name":           containerName,
				}
				envelope = map[string]any{
					"time":       time.Now().Format(time.RFC3339Nano),
				...
				labels["actordock.dev/actor_id"] = actorID
				labels["actordock.dev/actor_template_name"] = actorTemplateName
				labels["actordock.dev/actor_template_namespace"] = actorTemplateNamespace
				labels["actordock.dev/container_name"] = containerName
				envelope = m
			}
