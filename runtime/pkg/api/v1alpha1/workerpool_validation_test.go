...
func TestWorkerPoolValidation(t *testing.T) {
	...
	tests := []struct {
		...
		name: "missing sandboxImage",
		mutate: func(wp *WorkerPool) {
			wp.Spec.SandboxImage = ""
		},
		wantErr: true,
		errMsg:  "spec.sandboxImage: Invalid value: \"\": spec.sandboxImage in body should be at least 1 chars long",
	}, {
		name: "valid template",
		mutate: func(wp *WorkerPool) {
			wp.Spec.Template = &WorkerPoolPodTemplate{
				NodeSelector: map[string]string{"workload": "actordock"},
