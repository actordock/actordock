...
// KataConfig holds the values runtime-sandbox reads from a kata configuration.toml. runtime-sandbox
...
type KataConfig struct {
	...
	// etc.). runtime-sandbox appends these to the cloud-hypervisor payload cmdline, since
	// there is no kata shim to inject them.
	KernelParams string
}

// clhConfigTOML mirrors the subset of a kata configuration.toml runtime-sandbox reads.
...
// CPUs", which runtime-sandbox does not support).
