...
func (r *envResolver) resolve(ctx context.Context, containerName string, env runtimev1alpha1.EnvVar) (*runtimeworkerpb.EnvEntry, error) {
	envID := fmt.Sprintf("container %q env %q", containerName, env.Name)

	switch {
	case env.Value != nil:
		return &runtimeworkerpb.EnvEntry{
			Name:  env.Name,
			Value: *env.Value,
		}, nil
...
func (r *envResolver) resolveValueFrom(ctx context.Context, envID string, valueFrom *runtimev1alpha1.EnvVarSource) (string, bool, error) {
...
func (r *envResolver) resolveSecretKeyRef(ctx context.Context, envID string, ref *runtimev1alpha1.SecretKeySelector) (string, bool, error) {
	if r.kubeClient == nil {
