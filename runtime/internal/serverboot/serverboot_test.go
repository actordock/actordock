...
func TestNewResourceDefaults(t *testing.T) {
	res, err := newResource(context.Background(), "runtime-api")
	if err != nil {
		t.Fatalf("newResource: %v", err)
	}
	attrs := resourceAttrs(res)
	if got := attrs[string(semconv.ServiceNameKey)]; got != "runtime-api" {
		t.Errorf("service.name = %q, want runtime-api", got)
	}
