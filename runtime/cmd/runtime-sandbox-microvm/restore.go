...
//
// Contract with runtime-worker: the snapshot dir (config.json + state.json + memory-ranges +
...
// reconstruct each RO lower from the local OCI bundle (runtime-worker re-unpacked the golden
...
func (s *SandboxService) RestoreWorkload(ctx context.Context, req *runtimesandboxpb.RestoreWorkloadRequest) (resp *runtimesandboxpb.RestoreWorkloadResponse, retErr error) {
	...
	id := req.GetActorId()
	restoreDir := sandboxpath.RestoreStateDir(ns, name, id)
	tStart := time.Now()
	...
	// Reconstruct each container's overlay RO lower from the LOCAL OCI bundle (runtime-worker
	...
	// lifetime, so it must persist until teardown (runtime-worker keeps it until reset).
...
func rewriteSnapshotSocketPaths(snapshotDir, id string) error {
	...
	// runtime-sandbox captures the guest serial console to a file under the source actor's
