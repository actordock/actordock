...
// The validators in this file guard inputs that runtime-worker turns into host
// filesystem paths. runtime-worker listens on an insecure hostPort, so any reachable
// caller could otherwise smuggle a path separator or ".." through these
// fields and make runtime-worker read/RemoveAll/write outside the intended directory
...
// sandboxpath.ActorPath into a host path on which runtime-worker runs os.RemoveAll and
...
func ValidateActorRef(namespace, template, actorID string) error {
	...
	// (<namespace>:<template>:<actorID>, see sandboxpath.ActorPath), which must
...
// (static-files/runsc-<hash>) and, on a cache hit, returned for runtime-sandbox to
...
func ValidateRunscHash(sha256Hash string) error {
	...
	// Same decoder runtime-worker's digest comparison uses.
