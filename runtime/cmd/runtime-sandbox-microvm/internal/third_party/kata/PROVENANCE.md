...
and used by `cmd/runtime-sandbox-microvm`. The upstream `LICENSE` in this directory covers
everything under it.
...
when runtime-sandbox boots the guest.
...
runtime-sandbox-microvm drives a small subset of `AgentService`:
...
2. Set each `option go_package` to
   `github.com/actordock/runtime/cmd/runtime-sandbox-microvm/internal/third_party/kata/agentpb;agentpb`.
