...
func TestURL(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "default path",
			probe:   &runtimesandboxpb.Readyz{HttpGet: &runtimesandboxpb.HTTPGetAction{Port: 8080}},
			actorIP: "169.254.17.2",
			want:    "http://169.254.17.2:8080/readyz",
		},
		{
			name:    "explicit path",
			probe:   &runtimesandboxpb.Readyz{HttpGet: &runtimesandboxpb.HTTPGetAction{Path: "/health", Port: 9000}},
			actorIP: "169.254.17.2",
			want:    "http://169.254.17.2:9000/health",
		},
		{
			name:    "path without leading slash is normalized",
			probe:   &runtimesandboxpb.Readyz{HttpGet: &runtimesandboxpb.HTTPGetAction{Path: "ready", Port: 80}},
			actorIP: "10.0.0.1",
		...
		{
			name:    "port zero",
			probe:   &runtimesandboxpb.Readyz{HttpGet: &runtimesandboxpb.HTTPGetAction{Port: 0}},
			actorIP: "1.2.3.4",
			wantErr: true,
		},
		{
			name:    "port too large",
			probe:   &runtimesandboxpb.Readyz{HttpGet: &runtimesandboxpb.HTTPGetAction{Port: 70000}},
			actorIP: "1.2.3.4",
			wantErr: true,
		},
	}
...
func TestWait_ReturnsOnFirst200(t *testing.T) {
	...
	probe := &runtimesandboxpb.Readyz{HttpGet: &runtimesandboxpb.HTTPGetAction{Port: int32(port)}}
