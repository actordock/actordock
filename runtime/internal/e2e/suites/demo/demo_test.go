...
func createActor(ctx context.Context, t *testing.T, clients *e2e.Clients, nsObj *e2e.Namespace, at *v1alpha1.ActorTemplate) error {
	...
	createResp, err := clients.RuntimeAPI.CreateActor(ctx, &runtimeapipb.CreateActorRequest{
		ActorId:                actorID,
	...
	defer func() {
		clients.RuntimeAPI.DeleteActor(ctx, &runtimeapipb.DeleteActorRequest{
			ActorId: actorID,
		})
	}()

	listResp, err := clients.RuntimeAPI.ListActors(ctx, &runtimeapipb.ListActorsRequest{})
	if err != nil {
		t.Fatalf("ListActors RPC failed: %v", err)
	}

	var myActors []*runtimeapipb.Actor
	...
	if actor.Status != runtimeapipb.Actor_STATUS_SUSPENDED {
		t.Errorf("expected actor status to be SUSPENDED, got %v", actor.Status)
	}

	t.Logf("Successfully queried runtime API. Found %d active actors total, %d in our namespace %s.",
...
func pauseActor(ctx context.Context, t *testing.T, clients *e2e.Clients, nsObj *e2e.Namespace, at *v1alpha1.ActorTemplate) error {
	...
	if _, err := clients.RuntimeAPI.CreateActor(ctx, &runtimeapipb.CreateActorRequest{
		ActorId:                actorID,
		ActorTemplateNamespace: nsObj.Name,
		ActorTemplateName:      at.Name,
	}); err != nil {
	...
	waitForActorStatus(ctx, t, clients, actorID, runtimeapipb.Actor_STATUS_SUSPENDED)

	// Resuming the actor
	t.Logf("Resuming Actor %q...", actorID)
	if _, err := clients.RuntimeAPI.ResumeActor(ctx, &runtimeapipb.ResumeActorRequest{
		ActorId: actorID,
	}); err != nil {
		t.Fatalf("failed to resume Actor: %v", err)
	}
	waitForActorStatus(ctx, t, clients, actorID, runtimeapipb.Actor_STATUS_RUNNING)
	...
	if _, err := clients.RuntimeAPI.PauseActor(ctx, &runtimeapipb.PauseActorRequest{
		ActorId: actorID,
	}); err != nil {
		t.Fatalf("failed to pause Actor: %v", err)
	}
	waitForActorStatus(ctx, t, clients, actorID, runtimeapipb.Actor_STATUS_PAUSED)
	...
	if _, err := clients.RuntimeAPI.SuspendActor(ctx, &runtimeapipb.SuspendActorRequest{
		ActorId: actorID,
	}); err != nil {
	...
	if _, err := clients.RuntimeAPI.DeleteActor(ctx, &runtimeapipb.DeleteActorRequest{
		ActorId: actorID,
	}); err != nil {
		t.Fatalf("failed to delete Actor: %v", err)
	}
	// Verify deletion
	if _, err := clients.RuntimeAPI.GetActor(ctx, &runtimeapipb.GetActorRequest{
		ActorId: actorID,
	}); err == nil {
...
func createActorTemplate(ctx context.Context, t *testing.T, clients *e2e.Clients, nsObj *e2e.Namespace) (*v1alpha1.ActorTemplate, error) {
	...
	_, err = clients.RuntimeK8s.ApiV1alpha1().WorkerPools(nsObj.Name).Create(ctx, wp, metav1.CreateOptions{})
	if err != nil {
	...
	_, err = clients.RuntimeK8s.ApiV1alpha1().ActorTemplates(nsObj.Name).Create(ctx, at, metav1.CreateOptions{})
	if err != nil {
	...
	for {
		curAt, err := clients.RuntimeK8s.ApiV1alpha1().ActorTemplates(nsObj.Name).Get(tmplCtx, at.Name, metav1.GetOptions{})
		if err == nil {
...
func waitForActorStatus(ctx context.Context, t *testing.T, clients *e2e.Clients, actorID string, expectedStatus runtimeapipb.Actor_Status) {
	t.Helper()
	t.Logf("Waiting for Actor %q to be %v...", actorID, expectedStatus)
	timeout := 60 * time.Second
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		resp, err := clients.RuntimeAPI.GetActor(ctx, &runtimeapipb.GetActorRequest{
			ActorId: actorID,
		})
		if err == nil {
...
func callActor(t *testing.T, actorID string) (string, error) {
	...
	svc, err := clients.K8s.CoreV1().Services("actordock-system").Get(context.Background(), "runtime-net-router", metav1.GetOptions{})
	if err != nil {
		return "", fmt.Errorf("failed to get runtime-net-router service: %w", err)
	}

	selector := labels.SelectorFromSet(svc.Spec.Selector).String()
	pods, err := clients.K8s.CoreV1().Pods("actordock-system").List(context.Background(), metav1.ListOptions{LabelSelector: selector})
	if err != nil {
		return "", fmt.Errorf("failed to list runtime-net-router pods: %w", err)
	}
	if len(pods.Items) == 0 {
		return "", fmt.Errorf("no runtime-net-router pods found")
	}
	targetPod := pods.Items[0]

	config, err := runtimeclient.LoadConfig(e2e.KubeConfig, e2e.KubeContext)
	if err != nil {
	...
	reqHttp.Host = fmt.Sprintf("%s.actors.actordock.dev", actorID)
