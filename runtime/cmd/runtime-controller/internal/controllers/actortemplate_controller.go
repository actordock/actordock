...
type ActorTemplateReconciler struct {
	...
	RuntimeAPIClient runtimeapipb.ControlClient
}

//+kubebuilder:rbac:groups=actordock.dev,resources=actortemplates,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=actordock.dev,resources=actortemplates/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=actordock.dev,resources=actortemplates/finalizers,verbs=update
//+kubebuilder:rbac:groups=actordock.dev,resources=workerpools,verbs=get;list;watch;create;update;patch;delete
...
func (r *ActorTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	at := &runtimev1alpha1.ActorTemplate{}
	...
	case runtimev1alpha1.PhaseInitial:
		actorID := uuid.NewString()

		createReq := &runtimeapipb.CreateActorRequest{
			ActorId:                actorID,
			ActorTemplateNamespace: at.ObjectMeta.Namespace,
			ActorTemplateName:      at.ObjectMeta.Name,
		}
		_, err := r.RuntimeAPIClient.CreateActor(ctx, createReq)
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("while creating golden actor: %w", err)
		}

		at.Status.Phase = runtimev1alpha1.PhaseResumeGoldenActor
	...
	case runtimev1alpha1.PhaseResumeGoldenActor:

		// TODO(runtime-sandbox): If resumption fails because the runtime-sandbox or runtime-worker is not
		...
		resumeReq := &runtimeapipb.ResumeActorRequest{
			ActorId: at.Status.GoldenActorID,
		}
		_, err := r.RuntimeAPIClient.ResumeActor(ctx, resumeReq)
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("while resuming golden actor: %w", err)
		}

		at.Status.Phase = runtimev1alpha1.PhaseWaitGoldenActor
	...
	case runtimev1alpha1.PhaseWaitGoldenActor:
		...
		resp, err := r.RuntimeAPIClient.SuspendActor(ctx, req)
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("while suspending golden actor: %w", err)
		}

		if resp.GetActor().GetLatestSnapshotInfo().GetType() != runtimeapipb.SnapshotType_SNAPSHOT_TYPE_EXTERNAL {
		...
		at.Status.Phase = runtimev1alpha1.PhaseReady
		...
		return ctrl.Result{}, nil
	case runtimev1alpha1.PhaseReady:
		return ctrl.Result{}, nil
	default:
...
func (r *ActorTemplateReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).For(&runtimev1alpha1.ActorTemplate{}).Complete(r)
}
...
func goldenSnapshotWarmupFor(at *runtimev1alpha1.ActorTemplate) time.Duration {
