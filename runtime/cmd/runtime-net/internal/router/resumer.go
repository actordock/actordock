...
func NewActorResumer(apiClient runtimeapipb.ControlClient) *ActorResumer {
	return &ActorResumer{
		apiClient: apiClient,
	}
}
...
func (r *ActorResumer) ResumeActor(ctx context.Context, actorID string) (*runtimeapipb.Actor, error) {
	ch := r.flight.DoChan(actorID, func() (interface{}, error) {
		...
		var resumeResp *runtimeapipb.ResumeActorResponse

		err := wait.ExponentialBackoffWithContext(bgCtx, backoff, func(ctx context.Context) (bool, error) {
			var err error
			resumeResp, err = r.apiClient.ResumeActor(ctx, &runtimeapipb.ResumeActorRequest{
				ActorId: actorID,
			})
			if err == nil {
				return true, nil
	...
	case res := <-ch:
		...
		return res.Val.(*runtimeapipb.Actor), nil
	}
}
