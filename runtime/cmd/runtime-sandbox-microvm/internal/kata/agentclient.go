...
// AgentClient is a thin ttrpc client for the kata-agent RPCs runtime-sandbox drives
// directly. runtime-sandbox owns the cloud-hypervisor boot (no kata shim) and drives the
...
// this once at VM boot; runtime-sandbox (no shim) must call it itself so the agent has a
...
// runtime-sandbox does itself). The agent matches the link by HwAddr, then applies the
...
// ExecId, which runtime-sandbox sets equal to ContainerId.
...
// or stderr (stderr=true). execID equals containerID (runtime-sandbox sets ExecId ==
