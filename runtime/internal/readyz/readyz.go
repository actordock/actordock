...
// runtime-sandbox. The intent is to detect the moment a container's HTTP server
...
func WaitAll(ctx context.Context, containers []*runtimesandboxpb.Container, actorIP string) error {
...
func Wait(ctx context.Context, containerName string, probe *runtimesandboxpb.Readyz, actorIP string) error {
...
func URL(probe *runtimesandboxpb.Readyz, actorIP string) (string, error) {
	hg := probe.GetHttpGet()
	if hg == nil {
