...
type Claims struct {
	...
	Actordock ActordockClaims
}

type ActordockClaims struct {
	AppID     string
	UserID    string
	SessionID string
}
...
type WireClaims struct {
	...
	// Claims from ADK's session model.
	Actordock WireActordockClaims `json:"actordock.dev,omitempty"`
}

type WireActordockClaims struct {
...
func ClaimsToWire(claims *Claims) (*WireClaims, error) {
	...
	wire := &WireClaims{
		...
		JTI:        claims.JTI,
		Actordock: WireActordockClaims{
			claims.Actordock.AppID,
			claims.Actordock.UserID,
			claims.Actordock.SessionID,
		},
	}
