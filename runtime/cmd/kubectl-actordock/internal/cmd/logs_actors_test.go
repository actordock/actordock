...
func TestFilterAndDisplayLogLine(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			line:          `{"time":"2026-05-16T01:03:38.602878302Z","level":"info","msg":"Count","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-1"}}`,
			targetActorID: "act-1",
			wantMatched:   true,
		...
		},
		{
			...
			line:          `{"time":"2026-05-16T01:03:38Z","message":"Hello","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-1"}}`,
			targetActorID: "act-1",
			wantMatched:   true,
		...
		},
		{
			...
			line:          `{"level":"error","msg":"Failed","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-1"}}`,
			targetActorID: "act-1",
			wantMatched:   true,
			wantTime:      "",
		...
		},
		{
			...
			line:          `{"time":"2026-05-16T01:03:38.602878302Z","level":"info","msg":"Count","labels":{"actordock.dev/actor_id":"act-1"}}`,
			targetActorID: "act-1",
			wantMatched:   true,
			wantTime:      "2026-05-16T01:03:38.602878302Z",
			wantOutput:    `{"time":"2026-05-16T01:03:38.602878302Z","level":"info","msg":"Count"}`,
		},
		{
			...
			line:          `{"time":"2026-05-16T01:03:38Z","message":"Hello world","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-2"}}`,
			targetActorID: "act-1",
			wantMatched:   false,
			wantTime:      "2026-05-16T01:03:38Z",
			wantOutput:    "",
		...
		},
		{
			...
			line:          `{"time":"2026-05-16T01:03:38Z","level":"info","msg":"Hello","traceID":"abc-123","err":"timeout","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-1"}}`,
			targetActorID: "act-1",
			wantMatched:   true,
			wantTime:      "2026-05-16T01:03:38Z",
		...
		},
		{
			...
			line:          `{"time":"2026-05-16T01:03:38Z","severity":"error","message":"Disk full","custom_tag":"alert","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-1"}}`,
			targetActorID: "act-1",
			wantMatched:   true,
			wantTime:      "2026-05-16T01:03:38Z",
		...
		},
		{
			...
			line:          `{"message":"login failed","code":401,"logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-1"}}`,
			targetActorID: "act-1",
			wantMatched:   true,
			wantTime:      "",
		...
		},
		{
			...
			line:          `{"time":"2026-05-16T01:03:38Z","level":"info","msg":"Hello","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-1","app":"my-app"}}`,
			targetActorID: "act-1",
			wantMatched:   true,
			wantTime:      "2026-05-16T01:03:38Z",
...
type mockRuntimeAPIClient struct {
	GetActorFunc func(ctx context.Context, in *runtimeapipb.GetActorRequest, opts ...grpc.CallOption) (*runtimeapipb.GetActorResponse, error)
	CloseCalls   int
}

func (m *mockRuntimeAPIClient) GetActor(ctx context.Context, in *runtimeapipb.GetActorRequest, opts ...grpc.CallOption) (*runtimeapipb.GetActorResponse, error) {
...
func (m *mockRuntimeAPIClient) Close() {
	m.CloseCalls++
}
...
func TestLogsActorRunner_Run_OneShotSuccess(t *testing.T) {
	...
	logLine := `{"time":"2026-05-16T01:03:38Z","level":"info","msg":"Hello world","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-123"}}`
	...
	mockAPI := &mockRuntimeAPIClient{
		GetActorFunc: func(ctx context.Context, in *runtimeapipb.GetActorRequest, opts ...grpc.CallOption) (*runtimeapipb.GetActorResponse, error) {
			return &runtimeapipb.GetActorResponse{
				Actor: &runtimeapipb.Actor{
					ActorId: actorID,
					Status:  runtimeapipb.Actor_STATUS_SUSPENDED, // not running
				},
			}, nil
		},
	}
...
func TestLogsActorRunner_Run_Follow_SuspendedToRunning(t *testing.T) {
	...
	logLine := `{"time":"2026-05-16T01:03:38Z","level":"info","msg":"Follow hello","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-123"}}`
		...
		StreamLogsFunc: func(streamCtx context.Context, ns, name string, opts *corev1.PodLogOptions) (io.ReadCloser, error) {
			...
			if streamCalls == 1 {
				...
				go func() {
					...
					fmt.Fprintln(pw, `{"time":"2026-05-16T01:03:38Z","level":"info","msg":"line 1 from pod-1","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-migrate"}}`)
			...
			return io.NopCloser(strings.NewReader(`{"time":"2026-05-16T01:03:39Z","level":"info","msg":"line 1 from pod-2","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-migrate"}}` + "\n")), nil
		},
	}
	...
	mockStreamer := &mockPodLogsStreamer{
		StreamLogsFunc: func(streamCtx context.Context, ns, name string, opts *corev1.PodLogOptions) (io.ReadCloser, error) {
			...
			if streamCalls == 1 {
				pr, pw := io.Pipe()
				go func() {
					fmt.Fprintln(pw, `{"time":"2026-05-16T01:03:38Z","level":"info","msg":"before suspend","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-suspended-mid"}}`)
			...
			return io.NopCloser(strings.NewReader(`{"time":"2026-05-16T01:03:40Z","level":"info","msg":"after resume","logging.googleapis.com/labels":{"actordock.dev/actor_id":"act-suspended-mid"}}` + "\n")), nil
		},
	}
