...
func do(ctx context.Context) error {
	...
	slog.InfoContext(ctx, "runtime-sandbox booting")
	...
	// using AF_PACKET.
	interiorNetNS, err := createNetNSWithoutSwitching(ctx, sandboxpath.SandboxNetNSName(*podUID))
	if err != nil {
		return fmt.Errorf("while creating runtime-sandbox-interior netns: %w", err)
	}

	actorLogger := actorlog.NewActorLogger(syncedWriter, metadata.OnGCE())
	sandboxService := NewService(interiorNetNS, actorLogger)

	svr := grpc.NewServer(
		grpc.StatsHandler(otelgrpc.NewServerHandler()),
		grpc.UnaryInterceptor(runtimeinterceptors.ServerUnaryInterceptor),
	)
	runtimesandboxpb.RegisterSandboxServer(svr, sandboxService)
...
// SandboxService is a service for shepherding single microvm.
...
func NewService(interiorNetNS netns.NsHandle, actorLogger *actorlog.ActorLogger) *SandboxService {
	svc := &SandboxService{
...
func (s *SandboxService) RunWorkload(ctx context.Context, req *runtimesandboxpb.RunWorkloadRequest) (resp *runtimesandboxpb.RunWorkloadResponse, retErr error) {
	s.lock.Lock()
	defer s.lock.Unlock()

	s.actorLogger.EmitLifecycleLog("Actor starting", req.GetActorId(), req.GetActorTemplateName(), req.GetActorTemplateNamespace())

	// Contract with runtime-worker:
	//
	...
	// every line is tagged with the originating container (actordock.dev/container_name).
	...
	return &runtimesandboxpb.RunWorkloadResponse{}, nil
}

func (s *SandboxService) CheckpointWorkload(ctx context.Context, req *runtimesandboxpb.CheckpointWorkloadRequest) (*runtimesandboxpb.CheckpointWorkloadResponse, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	// Contract with runtime-worker:
	//
	//   * After we exit, runtime-worker will upload checkpoint to GCS
	//   * After we exit, runtime-worker will tear down OCI bundles and reset the actor directory.
	...
	checkpointPath := sandboxpath.CheckpointStateDir(req.GetActorTemplateNamespace(), req.GetActorTemplateName(), req.GetActorId())
	...
	switch req.GetScope() {
	case runtimesandboxpb.SnapshotScope_SNAPSHOT_SCOPE_DATA:
		var ddv []string
	...
	case runtimesandboxpb.SnapshotScope_SNAPSHOT_SCOPE_FULL:
	...
	// runtime-worker resets the actor runsc, bundle, pidfile, and checkpoint
	...
	// Report exactly the files runsc wrote so runtime-worker ships precisely this set
	...
	return &runtimesandboxpb.CheckpointWorkloadResponse{SnapshotFiles: snapshotFiles}, nil
}

// listSnapshotFiles returns the (relative) names of regular files directly under
// dir, which runtime-worker ships to object storage as the snapshot.
...
func (r *runsc) cleanupContainersAfterCheckpoint(ctx context.Context, containers []*runtimesandboxpb.Container) error {
...
func (s *SandboxService) RestoreWorkload(ctx context.Context, req *runtimesandboxpb.RestoreWorkloadRequest) (resp *runtimesandboxpb.RestoreWorkloadResponse, retErr error) {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	checkpointDir := sandboxpath.RestoreStateDir(req.GetActorTemplateNamespace(), req.GetActorTemplateName(), req.GetActorId())
	...
	return &runtimesandboxpb.RestoreWorkloadResponse{}, nil
}

func (s *SandboxService) setupActorNetwork(ctx context.Context) (retErr error) {
	...
	// sandbox0 as the gateway, and moves only the veth peer into the actor netns.
...
func (s *SandboxService) cleanupActorNetwork(ctx context.Context) error {
	// Remove all per-activation network state owned by runtime-sandbox. Deleting the
...
func (s *SandboxService) cleanupActorNetworkOrExit(ctx context.Context, msg string) {
...
func installActorNftablesRules(podIP net.IP) error {
	...
	// rules in an runtime-sandbox-owned table makes cleanup simple and avoids mutating
...
func removeActorNftablesRules() error {
	// Delete the whole runtime-sandbox nftables table if it exists. The table is
