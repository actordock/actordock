...
type Interface interface {
	...
	GetActor(ctx context.Context, id string) (*runtimeapipb.Actor, error)

	// Stores a new actor in suspended state. Returns ErrAlreadyExists if key is taken.
	CreateActor(ctx context.Context, actor *runtimeapipb.Actor) error
	...
	UpdateActor(ctx context.Context, actor *runtimeapipb.Actor, expectedVersion int64) error
	...
	ListActors(ctx context.Context, pageSize int32, pageToken string) ([]*runtimeapipb.Actor, string, error)

	// Fetches worker state by namespace, pool, and pod name. Returns ErrNotFound if missing.
	GetWorker(ctx context.Context, namespace, pool, pod string) (*runtimeapipb.Worker, error)

	// Registers a new idle worker. Returns ErrAlreadyExists if already registered.
	CreateWorker(ctx context.Context, worker *runtimeapipb.Worker) error
	...
	UpdateWorker(ctx context.Context, worker *runtimeapipb.Worker, expectedVersion int64) error
	...
	ListWorkers(ctx context.Context) ([]*runtimeapipb.Worker, error)
