...
const (
	routerNamespace = "actordock-system"
	routerService   = "runtime-net-router"
)

// RouterClient sends HTTP requests to actors through the runtime-net router, the
...
// approach in internal/runtimeclient.
...
// NewRouterClient establishes a port-forward to the runtime-net router. Call Close
// to tear it down.
func NewRouterClient(ctx context.Context) (*RouterClient, error) {
	config, err := runtimeclient.LoadConfig(KubeConfig, KubeContext)
	if err != nil {
