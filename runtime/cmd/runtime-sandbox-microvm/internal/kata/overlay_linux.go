...
func OverlayUpperBase(containerID string) string { return "/run/sandbox-upper/" + containerID }
...
func (a *AgentClient) CreateCarrier(ctx context.Context, cid string, spec *specs.Spec) error {
	...
	if pbSpec.Linux != nil {
		pbSpec.Linux.CgroupsPath = "/runtime-sandbox-chv/" + cid + "-carrier"
	}
...
func (a *AgentClient) StartOverlayWorkload(ctx context.Context, cid, workloadID, upperBase string, spec *specs.Spec) error {
	...
	// Per-workload cgroup: the shaped spec carries the actor-wide /runtime-sandbox-chv/<actorID>
	// (spec.go), which collides across an actor's containers — mirror the carrier's
	// per-id path so each workload gets its own cgroup.
	if pbSpec.Linux != nil {
		pbSpec.Linux.CgroupsPath = "/runtime-sandbox-chv/" + workloadID
	}
