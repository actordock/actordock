...
func TestWorkerPoolCreatesDeployment(t *testing.T) {
	wp := makeWorkerPool("test-create", "default", 3, "runtime-sandbox:v1")
	...
	eventually(t, func(ctx context.Context) (bool, error) {
		...
		if container.Image != "runtime-sandbox:v1" || container.Name != "runtime-sandbox" {
			return false, nil
		}
		if dep.Spec.Template.Labels["actordock.dev/worker-pool"] != wp.Name {
			return false, nil
		}
		...
		return len(dep.Spec.Template.Spec.Volumes) == 1 &&
			dep.Spec.Template.Spec.Volumes[0].Name == "run-runtime-sandbox", nil
	})
}
...
func TestWorkerPoolReplicasUpdate(t *testing.T) {
	wp := makeWorkerPool("test-replicas", "default", 2, "runtime-sandbox:v1")
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
	t.Cleanup(func() { k8sClient.Delete(testCtx, wp) }) //nolint:errcheck
...
// TestWorkerPoolImageUpdate verifies that changing spec.sandboxImage on a
// WorkerPool propagates to the managed Deployment.
func TestWorkerPoolImageUpdate(t *testing.T) {
	wp := makeWorkerPool("test-image", "default", 1, "runtime-sandbox:v1")
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
	t.Cleanup(func() { k8sClient.Delete(testCtx, wp) }) //nolint:errcheck
	...
	wp.Spec.SandboxImage = "runtime-sandbox:v2"
	if err := k8sClient.Update(testCtx, wp); err != nil {
	...
	eventually(t, func(ctx context.Context) (bool, error) {
		...
		return dep.Spec.Template.Spec.Containers[0].Image == "runtime-sandbox:v2", nil
	})
}
...
func TestSSAPreservesUnownedFields(t *testing.T) {
	wp := makeWorkerPool("test-ssa-unowned", "default", 2, "runtime-sandbox:v1")
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
	t.Cleanup(func() { k8sClient.Delete(testCtx, wp) }) //nolint:errcheck
...
func TestSSARevertsOwnedFields(t *testing.T) {
	wp := makeWorkerPool("test-ssa-owned", "default", 2, "runtime-sandbox:v1")
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
	t.Cleanup(func() { k8sClient.Delete(testCtx, wp) }) //nolint:errcheck
...
func TestDeletedDeploymentRecreated(t *testing.T) {
	wp := makeWorkerPool("test-recreate", "default", 2, "runtime-sandbox:v1")
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
	t.Cleanup(func() { k8sClient.Delete(testCtx, wp) }) //nolint:errcheck
...
func TestStatusReplicasPropagation(t *testing.T) {
	wp := makeWorkerPool("test-status", "default", 3, "runtime-sandbox:v1")
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
	t.Cleanup(func() { k8sClient.Delete(testCtx, wp) }) //nolint:errcheck
	...
	eventually(t, func(ctx context.Context) (bool, error) {
		current := &runtimev1alpha1.WorkerPool{}
...
func sampleWorkerPoolPodTemplate() *runtimev1alpha1.WorkerPoolPodTemplate {
	return &runtimev1alpha1.WorkerPoolPodTemplate{
		...
		PriorityClassName: "actordock-workers",
...
func TestWorkerPoolPodTemplatePropagation(t *testing.T) {
	wp := makeWorkerPool("test-template-propagate", "default", 1, "runtime-sandbox:v1")
	...
	eventually(t, func(ctx context.Context) (bool, error) {
		...
		if podSpec.NodeSelector["workload"] != "actordock" {
			return false, nil
		}
		if len(podSpec.Tolerations) != 1 || podSpec.Tolerations[0].Key != "nvidia.com/gpu" {
			return false, nil
		}
		if podSpec.PriorityClassName != "actordock-workers" {
			return false, nil
		}
...
func TestWorkerPoolPodTemplateUpdate(t *testing.T) {
	wp := makeWorkerPool("test-template-update", "default", 1, "runtime-sandbox:v1")
	wp.Spec.Template = sampleWorkerPoolPodTemplate()
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
	...
	eventually(t, func(ctx context.Context) (bool, error) {
		dep, err := getDeployment(ctx, wp)
		return err == nil && dep.Spec.Template.Spec.NodeSelector["workload"] == "actordock", nil
	})
...
func TestWorkerPoolPodTemplateClear(t *testing.T) {
	wp := makeWorkerPool("test-template-clear", "default", 1, "runtime-sandbox:v1")
	wp.Spec.Template = sampleWorkerPoolPodTemplate()
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
...
func TestWorkerPoolPodTemplateClearAll(t *testing.T) {
	wp := makeWorkerPool("test-template-clear-all", "default", 1, "runtime-sandbox:v1")
	wp.Spec.Template = sampleWorkerPoolPodTemplate()
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
	...
	eventually(t, func(ctx context.Context) (bool, error) {
		...
		podSpec := dep.Spec.Template.Spec
		container := podSpec.Containers[0]
		return podSpec.NodeSelector["workload"] == "actordock" &&
			len(podSpec.Tolerations) == 1 &&
			podSpec.PriorityClassName == "actordock-workers" &&
...
func TestSSARevertsOwnedPodTemplateFields(t *testing.T) {
	wp := makeWorkerPool("test-ssa-template", "default", 1, "runtime-sandbox:v1")
	wp.Spec.Template = sampleWorkerPoolPodTemplate()
	if err := k8sClient.Create(testCtx, wp); err != nil {
		t.Fatalf("create WorkerPool: %v", err)
	}
	...
	eventually(t, func(ctx context.Context) (bool, error) {
		...
		return dep.Spec.Template.Spec.NodeSelector["workload"] == "actordock", nil
	})
}
...
func TestReplicasValidationRejectsNegative(t *testing.T) {
	wp := makeWorkerPool("test-neg-replicas", "default", -1, "runtime-sandbox:v1")
...
func makeWorkerPool(name, ns string, replicas int32, image string) *runtimev1alpha1.WorkerPool {
...
func getDeployment(ctx context.Context, wp *runtimev1alpha1.WorkerPool) (*appsv1.Deployment, error) {
