...
func TestCache_SyncsOnStart(t *testing.T) {
	...
	if diff := cmp.Diff([]*runtimeapipb.Worker{w1, w2}, got, protocmp.Transform(), workerSortOpt); diff != "" {
...
func TestCache_CreatedEvent(t *testing.T) {
	...
	got, _ := c.Workers()
	if diff := cmp.Diff([]*runtimeapipb.Worker{w}, got, protocmp.Transform(), workerSortOpt); diff != "" {
		t.Errorf("workers mismatch (-want +got):\n%s", diff)
	}
}
...
func TestCache_UpdatedEvent_NewerVersionApplied(t *testing.T) {
	...
	got, _ := c.Workers()
	if diff := cmp.Diff([]*runtimeapipb.Worker{updated}, got, protocmp.Transform(), workerSortOpt); diff != "" {
		t.Errorf("workers mismatch (-want +got):\n%s", diff)
	}
}
...
func TestCache_UpdatedEvent_OlderVersionIgnored(t *testing.T) {
	...
	got, _ := c.Workers()
	if diff := cmp.Diff([]*runtimeapipb.Worker{w, sentinel}, got, protocmp.Transform(), workerSortOpt); diff != "" {
		t.Errorf("workers mismatch (-want +got):\n%s", diff)
	}
}
...
func TestCache_DeletedEvent(t *testing.T) {
	...
	fs.send(store.WorkerEvent{
		...
		Worker: &runtimeapipb.Worker{WorkerNamespace: "ns", WorkerPod: "pod1"},
	})
...
func TestCache_Relist_RecoversFromMissedDelete(t *testing.T) {
	...
	got, _ := c.Workers()
	if diff := cmp.Diff([]*runtimeapipb.Worker{w1}, got, protocmp.Transform(), workerSortOpt); diff != "" {
		t.Errorf("workers after relist (-want +got):\n%s", diff)
	}
}
...
func TestCache_Relist_FailureIsNonFatal(t *testing.T) {
	...
	if diff := cmp.Diff([]*runtimeapipb.Worker{w1}, workers, protocmp.Transform(), workerSortOpt); diff != "" {
		t.Errorf("workers mismatch (-want +got):\n%s", diff)
	}
}
...
func newFakeStore(workers ...*runtimeapipb.Worker) *fakeStore {
	return &fakeStore{
		workers: workers,
...
func (f *fakeStore) ListWorkers(_ context.Context) ([]*runtimeapipb.Worker, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.listErr != nil {
		return nil, f.listErr
	}
	out := make([]*runtimeapipb.Worker, len(f.workers))
	copy(out, f.workers)
	return out, nil
}
...
func (f *fakeStore) setWorkers(workers ...*runtimeapipb.Worker) {
	f.mu.Lock()
	f.workers = workers
	f.mu.Unlock()
}
...
func makeWorker(namespace, pod string, version int64) *runtimeapipb.Worker {
	return &runtimeapipb.Worker{
...
var workerSortOpt = cmpopts.SortSlices(func(a, b *runtimeapipb.Worker) bool {
