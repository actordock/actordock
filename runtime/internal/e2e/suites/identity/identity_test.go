...
func TestActorIdentity_AfterRestore_IsOwnID_NotGolden(t *testing.T) {
	...
	for _, id := range ids {
		...
		if got.File != id {
			t.Errorf("actor %q: /run/actordock/actor-id = %q, want %q (probe read error: %q)", id, got.File, id, got.Error)
		}
		if got.File == golden {
...
func deployProbe(t *testing.T, bucket string) {
	...
	// `-- --context=...` mirrors run_ko in hack/install-runtime.sh: ko's apply
...
func waitForGolden(t *testing.T, ctx context.Context, clients *e2e.Clients) string {
	...
	for time.Now().Before(deadline) {
		at, err := clients.RuntimeK8s.ApiV1alpha1().ActorTemplates(probeNamespace).Get(ctx, probeTemplate, metav1.GetOptions{})
		if err == nil {
...
func createAndResumeActor(t *testing.T, ctx context.Context, clients *e2e.Clients, id string) {
	...
	t.Cleanup(func() {
		...
		_, _ = clients.RuntimeAPI.SuspendActor(ctx, &runtimeapipb.SuspendActorRequest{ActorId: id})
		_, _ = clients.RuntimeAPI.DeleteActor(ctx, &runtimeapipb.DeleteActorRequest{ActorId: id})
	})

	// Resume from the golden snapshot (the restore path, not --boot).
	if _, err := clients.RuntimeAPI.ResumeActor(ctx, &runtimeapipb.ResumeActorRequest{ActorId: id}); err != nil {
