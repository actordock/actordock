...
type routerHealth struct {
	...
	apiClient runtimeapipb.ControlClient
	cfg       RouterConfig
}

func newRouterHealth(interval time.Duration, clientset kubernetes.Interface, apiClient runtimeapipb.ControlClient, cfg RouterConfig) *routerHealth {
	if interval <= 0 {
...
func (rh *routerHealth) check(ctx context.Context) {
	...
	// 3. Check runtime API gRPC
	{
		...
		if healthy {
			rh.report.RuntimeAPI.Healthy = true
			rh.report.RuntimeAPI.Message = msg
			rh.report.RuntimeAPI.LastSuccess = time.Now()
			rh.report.RuntimeAPI.SuccessCount++
		} else {
			rh.report.RuntimeAPI.Healthy = false
			rh.report.RuntimeAPI.Message = msg
...
func (rh *routerHealth) checkRuntimeAPI(ctx context.Context) (bool, string) {
	if rh.apiClient == nil {
	...
	_, err := rh.apiClient.ListActors(timeoutCtx, &runtimeapipb.ListActorsRequest{PageSize: 1})
	if err != nil {
		return false, err.Error()
	}
