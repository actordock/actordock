...
const (
	// SandboxClassGvisor is the gVisor/runsc runtime (cmd/runtime-sandbox). Default.
	SandboxClassGvisor SandboxClass = "gvisor"
	// SandboxClassMicroVM is the micro-VM runtime (cmd/runtime-sandbox-microvm); needs
	// /dev/kvm and vhost devices.
	SandboxClassMicroVM SandboxClass = "microvm"
)

// AssetFile is one content-addressed file that runtime-worker fetches for a sandbox
...
type AssetFile struct {
	...
	// fetched anonymously or with credentials depending on runtime-worker's
	// configuration.
	//
	// +required
...
type SandboxConfigSpec struct {
	...
	// Assets is the set of files runtime-worker fetches for this runtime, keyed first by
