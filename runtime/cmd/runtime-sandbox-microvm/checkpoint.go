...
//
// Contract with runtime-worker: after we return, runtime-worker uploads the checkpoint dir to object
// storage, then tears down bundles and resets the actor dir.
//
// runtime-sandbox drives the CH REST api-socket: pause -> snapshot file://<CheckpointStateDir>
...
func (s *SandboxService) CheckpointWorkload(ctx context.Context, req *runtimesandboxpb.CheckpointWorkloadRequest) (*runtimesandboxpb.CheckpointWorkloadResponse, error) {
	...
	// either way runtime-sandbox owns it and tracks its api-socket.
	ra := s.running[id]
	...
	checkpointDir := sandboxpath.CheckpointStateDir(ns, name, id)
	...
	// Report exactly the files we wrote so runtime-worker ships precisely the CH snapshot
	...
	// already on disk for runtime-worker to ship.
	tTeardown := time.Now()
...
// teardownActor stops the runtime-sandbox-owned CH VMM for an actor. Best-effort: the
// snapshot is already on disk, so this only needs to release resources. ra may be
// nil (e.g. runtime-sandbox restarted and lost in-memory state).
func (s *SandboxService) teardownActor(ctx context.Context, id string, ra *runningActor, client *ch.Client) {
	if client != nil {
		tShutdown := time.Now()
	...
	if ra != nil {
		...
		// Kill the CH process runtime-sandbox launched.
	...
	// processes. This is runtime-sandbox's own cleanup (process kill + unmount + rm).
