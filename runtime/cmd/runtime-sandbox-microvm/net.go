...
// Actor networking mirrors cmd/runtime-sandbox's veth model: a fresh
// point-to-point veth pair per activation, with the worker side (sandbox0,
...
//
// (Copied with light adaptation from cmd/runtime-sandbox; expected to be
...
const (
	...
	// runtime-sandbox where the kernel's random veth MAC is fine. A CH snapshot
...
// cleanupActorNetwork removes all per-activation network state owned by runtime-sandbox.
...
func installActorNftablesRules(podIP net.IP) error {
	// Dedicated runtime-sandbox-owned IPv4 table (cheap cleanup, no CNI chain mutation):
	...
	// Mirrors cmd/runtime-sandbox (same compatibility-bridge caveats and TODOs).
	if err := removeActorNftablesRules(); err != nil {
		return err
	}
...
// setupActorNetwork.
func (s *SandboxService) setupRestoreTap(ctx context.Context, name string, queuePairs int) ([]*os.File, error) {
	var fds []*os.File
...
// runtime-sandbox can configure the guest eth0 with a matching MTU via the agent
// (UpdateInterface). Defaults to 1500 if the link can't be read.
func (s *SandboxService) actorVethMTU(ctx context.Context) int {
	mtu := 1500
