...
func TestWrapContainerLogs(t *testing.T) {
	...
	if labels["actordock.dev/actor_id"] != "act-1" {
		t.Errorf("got actor_id = %v, want 'act-1'", labels["actordock.dev/actor_id"])
	}
	if labels["actordock.dev/actor_template_name"] != "tmpl-1" {
		t.Errorf("got actor_template_name = %v, want 'tmpl-1'", labels["actordock.dev/actor_template_name"])
	}
	if labels["actordock.dev/actor_template_namespace"] != "default" {
		t.Errorf("got actor_template_namespace = %v, want 'default'", labels["actordock.dev/actor_template_namespace"])
	}
	if labels["actordock.dev/container_name"] != "ctr-1" {
		t.Errorf("got container_name = %v, want 'ctr-1'", labels["actordock.dev/container_name"])
	}
}
...
func TestWrapContainerLogs_LabelCollision(t *testing.T) {
	input := `{"level":"info","msg":"App log","labels":{"actordock.dev/actor_id":"malicious-id","app":"my-app"}}` + "\n"
	rdr := strings.NewReader(input)
	...
	if labels["actordock.dev/actor_id"] != "act-1" {
		t.Errorf("got actor_id = %v, want 'act-1' (runtime metadata should take precedence)", labels["actordock.dev/actor_id"])
	}
}
