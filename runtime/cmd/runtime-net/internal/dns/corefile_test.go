...
func TestMakeCoreFile(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: []string{
				"actors.resources.runtime.actordock.dev:53 {",
				"log",
				"errors",
				"health :8080",
				"ready :8181",
				"reload",
				"template IN A actors.resources.runtime.actordock.dev {",
				`match "^` + resources.ActorIDRegexPattern + `\.actors\.resources\.runtime\.actordock\.dev\.$"`,
