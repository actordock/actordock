...
func (w *ActorWorkflow) ResumeActor(ctx context.Context, id string, boot bool) (*runtimeapipb.Actor, error) {
	input := &ResumeInput{
		ActorID: id,
		Boot:    boot,
	}
...
func (w *ActorWorkflow) SuspendActor(ctx context.Context, id string) (*runtimeapipb.Actor, error) {
	input := &SuspendInput{
		ActorID: id,
	}
	state := &SuspendState{}
...
func (w *ActorWorkflow) PauseActor(ctx context.Context, id string) (*runtimeapipb.Actor, error) {
	input := &PauseInput{
		ActorID: id,
	}
	state := &PauseState{}
