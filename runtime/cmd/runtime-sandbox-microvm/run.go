...
import (
	...
	"github.com/actordock/runtime/cmd/runtime-sandbox-microvm/internal/third_party/kata/agentpb"
...
// runningActor holds the live state for one actor's micro-VM. runtime-sandbox owns the
...
type runningActor struct {
	...
	// runtime-sandbox owns this CH process (booted at Run or relaunched at Restore).
	chCmd *exec.Cmd
	// vfsdCmd is the virtiofsd serving the overlay RO lower (the CH fs device
	// demand-pages from it for the actor's lifetime). runtime-sandbox owns it; teardownActor
	// kills it after the CH process.
	vfsdCmd *exec.Cmd
	// apiSocket is the CH api-socket for this runtime-sandbox-owned VMM.
	apiSocket string
...
// Asset names in RunWorkloadRequest.runtime_asset_paths (set by runtime-worker's
...
func (s *SandboxService) resolveRuntime(paths map[string]string) resolvedRuntime {
...
// bundle rootfs (the overlay RO lower) so the guest gets cluster DNS: runtime-sandbox drops
// runtime-worker's resolv.conf bind and sends no CreateSandbox.Dns, so the guest can
...
//
// runtime-sandbox boots cloud-hypervisor directly (no kata shim) and gives each container an
...
func (s *SandboxService) RunWorkload(ctx context.Context, req *runtimesandboxpb.RunWorkloadRequest) (resp *runtimesandboxpb.RunWorkloadResponse, retErr error) {
	...
	if len(containers) > maxActorContainers {
		return nil, status.Errorf(codes.Unimplemented, "runtime-sandbox-microvm supports at most %d containers, got %d", maxActorContainers, len(containers))
	}

	// runtime-sandbox builds the CH vm.create itself, so it needs the guest kernel + image
	// paths directly.
	paths := req.GetRuntimeAssetPaths()
	kernel, image := paths[assetKernel], paths[assetImage]
	if kernel == "" || image == "" {
		return nil, fmt.Errorf("runtime-sandbox-microvm requires %q and %q asset paths", assetKernel, assetImage)
	}
	...
	// the actor's lifetime, so runtime-sandbox owns the process (killed in teardownActor).
	...
	// Launch a bare VMM (CH + api-socket); runtime-sandbox owns this process for teardown.
...
func (s *SandboxService) buildActorContainers(ns, name, id string, containers []*runtimesandboxpb.Container) ([]actorContainer, error) {
	netnsPath := sandboxpath.SandboxNetNSPath(s.podUID)
	ctrs := make([]actorContainer, len(containers))
	for i, c := range containers {
		cn := c.GetName()
		bundle := sandboxpath.OCIBundlePath(ns, name, id, cn)
		...
		// Write cluster DNS into the lower before it's served over virtio-fs: runtime-sandbox
		// drops runtime-worker's resolv.conf bind and sends no CreateSandbox.Dns, so without
...
func (s *SandboxService) stageOverlayLowers(ctx context.Context, rr resolvedRuntime, id string, ctrs []actorContainer) (*exec.Cmd, error) {
	for _, c := range ctrs {
...
func (s *SandboxService) guestConfig(rr resolvedRuntime) (memMiB, vcpus int, kparams string, err error) {
	var cfgBytes []byte
	if rr.configFile != "" {
...
func (s *SandboxService) startActorContainers(ctx context.Context, ac *kata.AgentClient, id, vsockPath string, ctrs []actorContainer) error {
...
// each line with the actor's actordock.dev/* labels and writes it to the pod's stdout.
//
...
// (actordock.dev/container_name) so a multi-container actor demultiplexes.
...
// the connection.
func (s *SandboxService) startActorLogForwarding(ac *kata.AgentClient, actorID, streamID, containerName, name, ns string) {
...
func (s *SandboxService) configureGuestNetwork(ctx context.Context, ac *kata.AgentClient, mtu uint64) error {
