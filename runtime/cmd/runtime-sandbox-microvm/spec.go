...
// kata's OCI conversion requires but runtime-worker's (gVisor-oriented) spec omits.
// Without linux.resources, kata's ContainerConfig nil-derefs and the shim
// crashes. This shaper is a bridge; a future runtime-worker change should emit
...
func ensureKataCompatibleSpec(bundle, id, netnsPath string) (*specs.Spec, error) {
	...
	if spec.Linux.CgroupsPath == "" {
		spec.Linux.CgroupsPath = "/runtime-sandbox-chv/" + id
	}

	// runtime-worker's spec carries gVisor pause-model CRI annotations
	...
	// virtio-fs. runtime-sandbox assembles the actual overlay rootfs itself by driving the
	...
	// Replace runtime-worker's gVisor-oriented mounts (minimal /dev tmpfs, a
	...
	//
	// KNOWN GAP vs the gVisor runtime: this also drops runtime-worker's read-only actor
	// identity bind mount (/run/runtime/actor-id). The micro-VM guest can't see host
	// paths (the rootfs is an overlay of a virtio-fs base + a guest-RAM upper, not a
	// host bind), so runtime-worker's host-path identity mount has nothing to bind to.
