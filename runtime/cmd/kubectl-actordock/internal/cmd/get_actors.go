...
import (
	...
	"github.com/actordock/runtime/cmd/kubectl-actordock/internal/printer"
...
var getActorsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(args) > 0 {
			resp, err := apiClient.GetActor(ctx, &runtimeapipb.GetActorRequest{ActorId: args[0]})
			if err != nil {
		...
		for {
			resp, err := apiClient.ListActors(ctx, &runtimeapipb.ListActorsRequest{
				PageSize:  1000,
				PageToken: pageToken,
			})
			if err != nil {
