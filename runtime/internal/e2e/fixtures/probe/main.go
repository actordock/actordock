...
// assert on real in-actor state rather than the config runtime-worker generates.
//
...
// identityFile is the actor-id file inside the identity directory runtime-worker
// bind-mounts at IdentityMountPath.
const identityFile = "/run/actordock/actor-id"
