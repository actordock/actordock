...
type sandboxAssetsRecord struct {
	...
	// SnapshotFiles are the (relative) names of the files runtime-sandbox wrote into the
...
func recordFromRequest(sa *runtimeworkerpb.SandboxAssets) (*sandboxAssetsRecord, error) {
	if sa == nil {
...
func (s *WorkerHerder) ensureSandboxAssets(ctx context.Context, rec *sandboxAssetsRecord) (map[string]string, error) {
...
// returns immediately.
func (s *WorkerHerder) fetchAsset(ctx context.Context, entry assetEntry) (string, error) {
	if err := resources.ValidateRunscHash(entry.SHA256); err != nil {
		return "", status.Error(codes.InvalidArgument, err.Error())
	}

	localPath := sandboxpath.RunSCBinaryPath(entry.SHA256)
	...
	// runtime-worker-level decision, not per-asset: try the anonymous client first so the
...
func (s *WorkerHerder) openAsset(ctx context.Context, url string) (io.ReadCloser, error) {
	rc, anonErr := runtimegcs.Open(ctx, s.anonGCSClient, url)
	if anonErr == nil {
		return rc, nil
	}
	if s.gcsClient == nil {
	...
	rc, mainErr := runtimegcs.Open(ctx, s.gcsClient, url)
	if mainErr != nil {
...
func writeSandboxRecord(actorTemplateNamespace, actorTemplateName, actorID string, rec *sandboxAssetsRecord) error {
	...
	path := sandboxpath.ActorSandboxAssetsFile(actorTemplateNamespace, actorTemplateName, actorID)
