...
func (s *WorkerPoolSyncer) syncWorkerToStore(ctx context.Context, pod *corev1.Pod) {
	...
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			...
			err = s.persistence.CreateWorker(ctx, &runtimeapipb.Worker{
...
// in https://github.com/actordock/runtime/issues/23.
func (s *WorkerPoolSyncer) releaseActorOnDeadWorker(ctx context.Context, namespace, pool, podName string) error {
	...
	if actor.GetSandboxPodNamespace() != namespace || actor.GetSandboxPodName() != podName {
		return nil
	}
	actor.Status = runtimeapipb.Actor_STATUS_SUSPENDED
	actor.SandboxPodNamespace = ""
	actor.SandboxPodName = ""
	actor.SandboxPodIp = ""
